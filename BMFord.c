...
Result** BMFordAPSP_mt_b(const Graph* graph, int numthreads)
{
    ...
    pthread_mutex_t q_lock; pthread_mutex_init(&q_lock, NULL); // queue lock
    pthread_mutex_t i_lock; pthread_mutex_init(&i_lock, NULL); // iteration lock
    pthread_barrier_t i_barrier; pthread_barrier_init(&i_barrier, NULL, numthreads);
    ...
    pthread_mutex_destroy(&r_lock); pthread_mutex_destroy(&q_lock); pthread_mutex_destroy(&i_lock);
