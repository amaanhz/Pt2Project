...
FWarsh_args* construct_args(int nb, int r, int l, const int** d, const int** prev, int b1x, int b1y, int b2x,
    ...
    *args = (const FWarsh_args){
        .block_length = l, .m_dist = d, .m_prev = prev, .B1x = b1x, .B1y = b1y,
...
work_pool* init_work_pool(int nblocks) {
    ...
    *wp = (const work_pool){
        .items = malloc(sizeof(block_triplet) * nblocks * nblocks * nblocks),
...
void FWarsh_t(const void* args) {
    ...
    int** prev = a->m_prev;
    work_pool* wp = a->wp;
    pthread_mutex_t* wp_lock = a->wp_lock;
    pthread_mutex_t* dep_locks = a->dep_locks;
    pthread_cond_t* dep_conds = a->dep_conds;
...
Result** FWarsh_mt(const Graph* graph, int block_length, int numthreads) {
    ...
    *args = (const FWarsh_args_mt){
        .block_length = block_length, .num_blocks = num_blocks, .rem = rem, .deps = deps,
